...
function checkPrereqs() {
  ...
  # 主要就是用于比较可执行程序的版本和docker镜像的版本号是否一致，避免不兼容问题
  LOCAL_VERSION=$(peer version | sed -ne 's/ Version: //p')  # 提取出版本号
  # 获取当前镜像的版本号，并截取
  ...
  # 本地可执行程序的版本和BLACKLISTED_VERSIONS中的版本是否一致，其中BLACKLISTED_VERSIONS表示的是不支持的版本号
  # BLACKLISTED_VERSIONS表示的是不支持测试网络的版本，目的就是判断本地版本是否支持该测试网络
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    # 该echo中的grep -q类似于if语句，判断当前本地版本和所支持的版本是否一致，不一致则继续执行
...
function createConsortium() {
  ...
  # 根据联盟配置文件，生成创世块，指定orderer使用的channel为system-channel，属于系统channel
  # 相当于是生成系统通道的创世块
  set -x
...
## call the script to join create the channel and join the peers of org1 and org2s
...
## Call the script to isntall and instantiate a chaincode on the channel
function deployHospitalCC() {

  scripts/deployHospitalCC.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
if [ "$MODE" == "up" ]; then
  ...
  echo
elif [ "$MODE" == "deployHospitalCC" ]; then
  echo "deploying chaincode on channel '${CHANNEL_NAME}'"
  echo
else
  printHelp
