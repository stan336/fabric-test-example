...
# 创建应用通道 mychannel 的初始区块的交易文件 mychannel.tx，用于后续生成通道使用
# 类似一个凭证
createChannelTx() {
...
# 生成一个更新锚点的更新channel配置信息
# Org1MSP和Org2MSP分别为两个字符串标识别
createAncorPeerTx() {
...
# 当前组织创建了channel，但是并没有加入该channel，后续需要手动加入channel
createChannel() {
	setGlobals 1
	# Poll in case the raft leader is not set yet
	# 这里多次尝试生成channel，因为有可能raft共识还没开启
	# 生成当前用户自定义的通道的创世块
	# 只能由orderer节点创建channel
	local rc=1
	local COUNTER=1
...
joinChannel() {
  ...
  setGlobals $ORG
    ...
    sleep $DELAY
    set -x
    # 需要先设置对应的环境变量配置等，才能peer，它根据相应变量将对应节点加入到通道
