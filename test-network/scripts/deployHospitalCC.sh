...
CC_NAME="hospital_recordInfo"
CC_ROOT_PATH="../chaincode/${CC_NAME}/go"

if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	CC_RUNTIME_LANGUAGE=golang
	CC_SRC_PATH=${CC_ROOT_PATH}
...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${VERSION} --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} >&log.txt
  set +x
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeInvoke(){
  ...
  set -x
  ## 传入数组参数
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} --peerAddresses localhost:7051 --peerAddresses localhost:9051 --tlsRootCertFiles ${PEER0_ORG1_CA} --tlsRootCertFiles ${PEER0_ORG2_CA} -c '{"Args":["Save","110117","zhangsan","[\"110114\",\"110115\"]","110114"]}' >&log.txt
  # peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} --peerAddresses localhost:7051 --peerAddresses localhost:9051 --tlsRootCertFiles ${PEER0_ORG1_CA} --tlsRootCertFiles ${PEER0_ORG2_CA} -c '{"Args":["Save","110117","zhangsan","110114","13711111111"]}' >&log.txt
  res=$?
  set +x
  cat log.txt
  verifyResult $res "Invoke execution on $PEERS failed "
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["QueryHistoryRecord","110117"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
